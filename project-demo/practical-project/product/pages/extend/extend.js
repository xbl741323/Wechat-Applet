...
Page({
  ...
  draw() {
    ...
    let data = {
      ...
      list: [{
        type: 'wxml',
        class: '.code-contain, .share-img-sty, .share-text, .share-bg, .share-text-box, .share-title, .price-num-box, .price-num, .share-text, .cover-img',
        limit: '.code-contain',
        x: this.data.system == 'android' ? -18 : -11.5,
        y: 0
      }]
    }
    //传入数据，画制canvas图片
