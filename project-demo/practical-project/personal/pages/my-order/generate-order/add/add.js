...
Page({
  ...
  validatePhone() {
    ...
    if ((/^1[345789]\d{9}$/.test(signManInfo.mobile))) {
  ...
  returnParams() {
    ...
    let params = {
      orderStatus: orderStatus, // -1-待生效 -2-草稿
      clueType: clueType,
  ...
  generateFinalNewOrder(params) {
    ...
    generateOrder(params).then(res => {
      if (res.code == CodeMsg.CODE_0) {
        ...
        let orderId = res.data.tmOrderVoList[0].id
        setTimeout(() => {
          let wxCodeUrl = `/personal/pages/my-order/generate-order/show-code/show-code?id=${orderId}`
          let orerListUrl = `/personal/pages/my-order/list/list?type=${3}`
          wx.redirectTo({
            url: orderStatus == -2 ? orerListUrl : ((selectProduct.length == 1) && (auditStatus == false)) ? wxCodeUrl : orerListUrl,
          })
        }, 500)
      } else {
        wx.showToast({
  ...
  getUnitInfo() {
    ...
    getByOrganization(params).then(res => {
      if (res.code == CodeMsg.CODE_0) {
        let signManInfo = this.data.signManInfo
        console.log(signManInfo, "signManInfo")
        let result = res.data
        if (result != null && result.length > 0) {
          ...
          this.setData({
            phoneStatus: valideResult.length > 0 ? valideResult[0].status == 2 ? 2 : 0 : 0
          })
        } else {
          this.setData({
            phoneStatus: 0
