...
import { getUserDetail, getQrCode } from "../../utils/api/personal"
...
Page({
  ...
  // 获取订阅url
  getUrl(){
    getQrCode({number: wx.getStorageSync('userInfo').number}).then(res=>{
  ...
  getUserDetail(){
    ...
    if(wx.getStorageSync('userInfo')){
      getUserDetail({id:wx.getStorageSync('userInfo').id}).then(res=>{
        if(res.code == CodeMsg.CODE_0){
          let data = res.data
