...
Page({
  ...
  data: {
    ...
    orderId: "", // 订单id
    orderStatus: -1, // 订单状态 -1-待生效 -2-草稿 -5-不通过
  ...
  returnParams() {
    ...
    let params = {
      ...
      orderStatus: orderStatus, // -1-待生效 -2-草稿 -5-不通过
    ...
    updateOrder(params).then(res => {
      if (res.code == CodeMsg.CODE_0) {
        ...
        setTimeout(() => {
          ...
          wx.redirectTo({
            url: (orderStatus == -2 || orderStatus == -5) ? orerListUrl : ((selectProduct.length == 1) && (auditStatus == false)) ? wxCodeUrl : orerListUrl,
          })
        }, 500)
      }
    })
    ...
    getByOrganization(params).then(res => {
      if (res.code == CodeMsg.CODE_0) {
        ...
        if (result != null && result.length > 0) {
          ...
          this.setData({
            phoneStatus: valideResult[0].status == 2 ? 2 : 0
          })
        } else {
          this.setData({
            phoneStatus: 0
