...
Page({
   ...
   */
  data: {
    isAutoConversion: false, // 是否已认证 true-是 false-否
    clueInfo: {}, // 线索对象
    isPerson: true,
    clueId: "", // 编辑时的线索id
  ...
  fixParams() {
    ...
    let salesManInfo = this.selectComponent("#basic").properties.salesManInfo
    let params = {
    ...
    if (operateType == 0) {
      params.clueSalesmanEmpNo = salesManInfo.salesManNo // 业务员No
      params.clueSalesmanEmpId = salesManInfo.salesManId // 业务员id
      params.clueSalesmanEmpName = salesManInfo.salesManLabel // 业务员name
    } else {
  ...
  operateFinalClue: throttle(function () {
    ...
    let clueType = this.data.clueType
    let operateType = this.data.operateType
    console.log(this.fixParams(), "this.fixParams()")
  ...
  receiveContactInfo() {
    ...
    if (clueInfo.clueCompanyContactList && clueInfo.clueCompanyContactList.length > 0) {
      ...
      basicInfo.contactNameStatus = objectOne.authStatus == 2 ? true : false
      basicInfo.contactMobileStatus = (objectOne.authStatus == 2 || objectOne.authStatus == 1) ? true : false
      this.basic.setData({
        basicInfo: basicInfo
      })
  ...
  receiveUnitInfo() {
    ...
    if (clueType == 2) {
      ...
      if (clueInfo.clueCompanyUnit) {
        ...
        unitData.inventionPatentCount = clueOrganization.inventionNum // 发明专利数量
        unitData.usefulPatentCount = clueOrganization.practicalNum // 实用专利数量
        unitData.facePtentCount = clueOrganization.appearanceNum // 外观专利数量
        unitData.trademarkCount = clueOrganization.trademarkNum // 商标数量
        unitData.copyrightCount = clueOrganization.copyrightNum // 版权数量
        unitData.otherPropertyCount = clueOrganization.otherNum // 其他知产数量
        unitData.lastYearSalesCount = Number(clueOrganization.lastYearSale) == 0 ? '' : Number(clueOrganization.lastYearSale) // 上年度销售总额id
        unitData.lastYearSalesName = transInfo.lastYearSale // 上年度销售总额名称
        unitData.lastYearSalesCountDetail = clueOrganization.lastYearSaleAmount
        unitData.lastYearCapitalCount = Number(clueOrganization.lastYearAssets) == 0 ? '' : Number(clueOrganization.lastYearAssets) // 上年度资产总计id
        unitData.lastYearCapitalName = transInfo.lastYearAssets // 上年度资产总计名称
        unitData.lastYearCapitalCountDetail = clueOrganization.lastYearAssetsAmount
        unitData.lastYearProfitCount = Number(clueOrganization.lastYearProfit) == 0 ? '' : Number(clueOrganization.lastYearProfit) // 上年度利润总额id
        unitData.lastYearProfitName = transInfo.lastYearProfit // 上年度利润总额名称
        unitData.lastYearProfitCountDetail = clueOrganization.lastYearProfitAmount
        unitData.lastYearDevelopCount = Number(clueOrganization.lastYearDevExpenses) == 0 ? '' : Number(clueOrganization.lastYearDevExpenses) // 上年度研费总额id
        unitData.lastYearDevelopName = transInfo.lastYearDevExpenses // 上年度研费总额名称
        unitData.lastYearDevelopCountDetail = clueOrganization.lastYearDevExpensesAmount
        unitData.unitPersonCount = Number(clueOrganization.unitEmployees) == 0 ? '' : Number(clueOrganization.unitEmployees) // 单位人数
        unitData.unitPersonName = transInfo.unitEmployees // 单位人数名称
        unitData.unitPersonCountDetail = clueOrganization.unitEmployeesNum
        unitData.mainBusiness = clueOrganization.mainBusiness // 主营业务
        unitData.nextProjectPlan = clueOrganization.nextPlanning // 规划        
      }
      // 人员信息
  ...
  receiveBasicInfo() {
    ...
    this.setData({
      isAutoConversion: clueInfo.isAutoConversion
    })
  },
  // 回显基本信息校验值
